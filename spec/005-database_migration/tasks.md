...
- [x] **Task 4.2:** End-to-end cross-link verification
  ...
  - Depends on: 4.1
  - **Result (2026-05-26):** All eight explicit assertions pass; plus the design's per-page "Cross-links (inbound/outbound)" subsections (lines 219–228, 380–388, 517–521) all check out modulo one gap that was fixed in-place. Per-backend Outbox pages (`MSSQLOutbox.md` line 11+37, `MySQLOutbox.md` line 11+37, `PostgresOutbox.md` line 11+37, `SqliteOutbox.md` line 11+37) each link to both `BoxProvisioning.md` and `BoxProvisioningConfiguration.md`. Per-backend Inbox pages (`MSSQLInbox.md` line 38, `MySQLInbox.md` line 37, `PostgresInbox.md` line 38, `SqliteInbox.md` line 38) each link to both. `BoxProvisioning.md` support matrix (line 99) + Further Reading (lines 147–148) link to all 8 per-backend pages. `BoxProvisioning.md` differences table (line 107) links to `BoxProvisioningUpgrade.md#mssql-multi-version-upgrades`, plus a body reinforcement at line 115. `BrighterOutboxSupport.md` line 198 and `BrighterInboxSupport.md` line 73 both link to `BoxProvisioning.md`. All 5 Glossary anchors used in `BoxProvisioning.md` (`#boxprovisioning`, `#advisory-lock`, `#migration-history-table`, `#bootstrap-path`, `#migration-chain`) resolve to H3 headings under `## Database Provisioning` in `Glossary.md` (lines 171, 177, 183, 189, 195). Sample link in `BoxProvisioningConfiguration.md` line 245 points to `Brighter/samples/WebAPI/WebAPI_Dapper/GreetingsWeb/Startup.cs:116` (Task 1.2-confirmed line). All ADR references across the three new pages use the `Brighter/docs/adr/{0053,0057}-...md` relative form per CLAUDE.md External Links convention. Orphan check: all three new files reachable from `SUMMARY.md` lines 89–91. Python-based broken-link/broken-anchor scan over all `/contents/` links in the three new pages returned **0 broken**. **One fix made**: added a `[Glossary]` entry to `BoxProvisioning.md`'s Further Reading section (line 150 — between `OutboxPattern` and the ADR line) to satisfy design.md line 224's explicit Further Reading inventory; the body already had 5 inline anchor links, so this is the consolidated entry that mirrors the same line in `BoxProvisioningUpgrade.md` Further Reading.

- [x] **Task 4.3:** CLAUDE.md QA checklist pass + close-out
  ...
  - Depends on: 4.2
  - **Result (2026-05-26):** QA checklist completed. **Code Quality** — extension method names (`AddMsSqlOutbox/Inbox`, `AddPostgreSqlOutbox/Inbox`, `AddMySqlOutbox/Inbox`, `AddSqliteOutbox/Inbox`, `AddSpannerOutbox/Inbox`) all confirmed against `../Brighter/src/Paramore.Brighter.BoxProvisioning.*/`; `BoxProvisioningOptions.MigrationLockTimeout` property still on `BoxProvisioningOptions.cs:74`; SQLite `enableWalMode = true` default still on `SqliteBoxMigrationRunner.cs:97`; sample `Startup.cs:116` still contains the `UseBoxProvisioning` call. All 25 fenced code blocks across the three new pages have language tags (`csharp`/`sql`/`text`/`powershell`). **Content Quality** — three runner-path terms (fresh install, bootstrap, normal migration) used consistently; `__BrighterMigrationHistory` long-form on first use of all three new pages with the Spanner `BrighterMigrationHistory` exception called out; "degenerate runner" / "fresh-install-only" used for Spanner (never "broken" or "unsupported"); Option B framed as "manage the DDL yourself" everywhere — all 8 per-backend pages contain the affirming "Neither option is deprecated" line and no instances of "legacy"/"old way" attached to Option B. **Structure** — `SUMMARY.md` carries the new "Database Provisioning" section (Task 4.1); cross-link verification (Task 4.2) zero broken; heading depth ≤ H4 on every new page; all three new pages carry a Further Reading section. **Technical Accuracy** — code surface re-verified against source per Task 1.1 drift findings (no `migrationLockTimeout` parameter on `UseBoxProvisioning`; lock timeout late-bound; SQLite `enableWalMode`; SQLite + Spanner have no `schemaName`; single-call contract); discriminator-gate error messages still match ADR 0057 §2. **Fixes applied in this task:** three lowercase `outbox`/`inbox` → `Outbox`/`Inbox` capitalisations in `BoxProvisioning.md` (lines 85, 116, 126) where they refer to Brighter concepts; lowercase findings in `BrighterOutboxSupport.md`/`BrighterInboxSupport.md` are pre-existing prose from earlier authors (outside Task 4.3 scope — spec-005 only added the Task 2.4 signpost paragraphs which use proper capitalisation). README status checklist updated; PR #63 ready to merge.
